...
<template>
  <button
    @click="toggleCart"
    class="p-2 rounded-full hover:bg-gray-100 absolute top-4 right-4 z-50"
  >
    <v-icon class="block text-base-content h-8 w-8">mdi-cart</v-icon>
    <span class="bg-green-600 px-2 py-1 rounded-full text-xs text-white ml-2">{{
      cartItems.length
    }}</span>
  </button>
  <div
    class="fixed top-0 right-0 z-40"
    :class="{ 'w-full h-full': isMobile, 'w-1/2': !isMobile, hidden: !isOpen }"
  >
    <div class="bg-white h-full shadow-xl overflow-auto p-4">
      ...
      >
        ...
        <button @click.stop="removeItem(item.id)" class="text-red-500 text-sm">
          Remover
        </button>
      </div>
      <div class="mt-4">
